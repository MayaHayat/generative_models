"""
Stage 2 improved CovidGAN generator/discriminator. Two changes vs.
covidgan/models.py, both targeting generation quality (evaluated via FID,
not classifier accuracy -- see README's Stage 2 notes for why):
...
1. Spectral normalization on every discriminator layer (replacing plain
   ...
   GANs"). Constrains the discriminator's Lipschitz constant, which
   stabilizes AC-GAN adversarial training -- directly targets the D/G loss
   oscillation observed across every training run of the baseline model.
...
2. kernel_size=4, stride=2 transpose convolutions in the generator
   ...
   stride 2; 5 is not, which is exactly the condition that produces
   checkerboard artifacts in transposed-conv upsampling (Odena et al. 2016,
   "Deconvolution and Checkerboard Artifacts") -- directly observed in this
   project's own sample grids.

Same overall topology, channel widths, and roughly the same parameter count
as the baseline in models.py, so FID / sample-quality comparisons between
the two are apples-to-apples -- only these two mechanisms differ.
"""
import torch
